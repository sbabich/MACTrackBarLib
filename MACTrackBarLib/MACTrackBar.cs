...
namespace XComponent.SliderBar
{
	...
	{	
        ...
        private Color _bufferedColor = Color.Red;
        public RectangleF _bufferedRect = RectangleF.Empty;
		...
		protected override void OnPaint(System.Windows.Forms.PaintEventArgs e)
		{
			...
			if(_orientation == Orientation.Horizontal)
			{
				...
				if (_maximum == _minimum)
                    ...
                    currentTrackerPos = Convert.ToSingle(workingRect.Width - _trackerSize.Width) * Convert.ToSingle(_value - _minimum) / Convert.ToSingle(_maximum - _minimum) + workingRect.Left;

                float currentBufferPos;
                if (_maximum == _minimum)
                    currentBufferPos = workingRect.Left;
                else
                    currentBufferPos = Convert.ToSingle(workingRect.Width - _trackerSize.Width) * Convert.ToSingle(_buffered - _minimum) / Convert.ToSingle(_maximum - _minimum);
                ...
                //_bufferedRect = new RectangleF(workingRect.Left + _trackerSize.Width / 4, currentUsedPos + _trackerSize.Height / 2 - _trackerSize.Height / 4, currentBufferPos, _trackerSize.Height / 2 + 1);
                _bufferedRect = new RectangleF(workingRect.Left + _trackerSize.Width / 2, 
				...
				// Draw the Track Line
				//==========================================================================
                drawRect = new RectangleF(workingRect.Left + _trackerSize.Width / 2, 
                    currentUsedPos + _trackerSize.Height / 2 - _trackLineHeight / 2, 
                    workingRect.Width - _trackerSize.Width, 
                    _trackLineHeight);
                
                DrawTrackLine(e.Graphics, drawRect, _bufferedRect, _bufferedColor);
				currentUsedPos += _trackerSize.Height;
			...
			else //_orientation == Orientation.Vertical
			{
                ...
                float currentBufferPos;
                if (_maximum == _minimum)
                    currentBufferPos = workingRect.Top;
                else
                    currentBufferPos = (workingRect.Height - _trackerSize.Width) * (_buffered - _minimum) / (_maximum - _minimum);
                ...
                DrawTrackLine(e.Graphics, drawRect, RectangleF.Empty, _bufferedColor);
				currentUsedPos += _trackerSize.Height;
				//==========================================================================
        ...
        private void DrawTrackLine(Graphics g, RectangleF drawRect, RectangleF bufferedRect, Color bufferedColor)
		{
            DrawMACStyleHelper.DrawAquaPillSingleLayer(g, drawRect, _trackLineColor, _orientation, bufferedRect, bufferedColor);
		}
